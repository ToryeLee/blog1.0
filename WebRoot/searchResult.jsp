...
obj.id='selected'
      	...
      	<form action="SearchServlet?n=5" method="post" >
      	...
      	<c:if test="${requestScope.flag==0 }"><script>alert("抱歉，暂时没有找到有关该文章的信息！")</script><h6>博主暂时没有写过这样的文章！</h6></c:if>
      	<c:if test="${requestScope.flag!=0 }">

  	   <%
  	   		String size=(String)request.getAttribute("size");
  	   		System.out.println("444444 size is "+size);
  	   		int pageCount = 10;//显示每页记录数
		    	
		    int totalCount=Integer.parseInt(size);//页数获取总
       ...
       <table width="500" border="0">  
			<c:forEach items="${requestScope.Alist}" var="article">
				<li>
				<h3><a href="showArticle.jsp?articleid=${articlsessionScope}">${article.title }</a></h3>
				<p>${article.intro }</p>
