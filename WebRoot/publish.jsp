...
<form action="PublishServlet" method="post">
	<div class="mid">
    <h3>发表新博客</h3>
		<p>标题<input type="text" name="title" maxlength=40 class="title" size="120" value="${requestScope.title }"/><br/>
        <p>类别
        <select name="atype">
        <option value="${requestScope.atype }"  >${requestScope.atype }</option>
        	<option value="web类"  >web类</option>
            <option value="java类"  >java类</option>
            <option value="数据库类"  >数据库类</option>
            <option value="日记类"  >日记类</option>
        </select><br/>
        <p>简介<textarea name="intro" rows="5" cols="85" onKeyUp="if(this.value.length>100)this.value=this.value.substr(0,100)" >${requestScope.intro }</textarea>
        <p>内容</p>
		<textarea name="content" rows="25" cols="85" onKeyUp="if(this.value.length>80000)this.value=this.value.substr(0,80000)" >${requestScope.content }</textarea>
        <input type="submit" name="submition" value="发表" class="in" />
